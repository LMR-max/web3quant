...
- **绝不**在源码中硬编码密钥。
- 使用 `crypto_data_system/local_secrets.json`（已被 .gitignore 忽略）或环境变量注入。
- 发布前必须确认本地私钥文件未被 Git 跟踪。

**数据与缓存**

- 原始数据、缓存、模型、日志属于运行产物，默认不入库。
- 建议永远忽略：`data/`、`data_manager_storage/`、`logs/`、`models/`、`machine_learning/outputs/`、`*.parquet/*.pkl/*.zip`。

**发布前最低检查**

1. `git status --porcelain` 必须为空
2. secrets 扫描无命中（除本地私有文件）
3. 抽查 .gitignore 覆盖所有数据与缓存目录
